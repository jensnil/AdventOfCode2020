...
fn print(space : &HashMap<(i32,i32,i32,i32),char>) {
    ...
    let min_w = space.keys().map(|&it| { it.3 }).min().unwrap();
    let max_w = space.keys().map(|&it| { it.3 }).max().unwrap();
    for space_w in min_w..=max_w {
        for space_z in min_z..=max_z {
            println!("z = {}, w = {}", space_z, space_w);
            for space_y in min_y..=max_y {
                for space_x in min_x..=max_x {
                    if space.contains_key(&(space_x, space_y, space_z, space_w)) {
                        print!("#");
                    } else {
                        print!(".");
                    }
...
fn main() {
    ...
    let mut space = input.into_iter().enumerate()
        .map(|line| {
            line.1.chars().enumerate().filter(|&it| { it.1 == '#' })
                .map(|column| { ((column.0 as i32, line.0 as i32, 0_i32, 0_i32), column.1) })
                .collect::<Vec<(_,_)>>()
        })
        .flatten()
    ...
    for _ in 1..=6 {
        //print(&space);
        let mut next_space : HashMap<(i32,i32,i32,i32),char> = HashMap::new();
        ...
        for space_x in min_x-1..=max_x+1 {
            for space_y in min_y-1..=max_y+1 {
                for space_z in min_z-1..=max_z+1 {
                    for space_w in min_w-1..=max_w+1 {
                        let position = (space_x, space_y, space_z, space_w);
                        ...
                        let w = (space_w - 1..=space_w + 1).collect::<Vec<i32>>();
                        let active_counter = x.iter()
                            .cartesian_product(y.iter())
                            .cartesian_product(z.iter())
                            .cartesian_product(w.iter())
                            .map(|it| { (*it.0.0.0, *it.0.0.1, *it.0.1, *it.1) })
